...
def get_next_token_punctuation_mask(tokens: torch.LongTensor, model: HookedTransformer, fill_last_pos=True) -> torch.BoolTensor:
    ...
    if not fill_last_pos:
        return next_token_punctuation_mask[:-1]
...
def orthogonal_vector_decomposition(a: torch.Tensor, b: torch.Tensor):
    ...
    a_collinear = torch.dot(a, b) / torch.norm(b).pow(2) * b
    a_orthogonal = a - a_collinear
    return a_collinear, a_orthogonal

def get_orthogonal_component(a: torch.Tensor, b: torch.Tensor):
    '''Remove the component of a that is collinear with b'''
    projection = torch.dot(a, b) / torch.norm(b).pow(2) * b
    return a - projection

def get_collinear_component(a: torch.Tensor, b: torch.Tensor):
