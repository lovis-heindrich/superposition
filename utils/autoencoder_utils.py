...
def encoder_dla_batched(tokens: Int[Tensor, "batch pos"], model: HookedTransformer, encoder: AutoEncoder, cfg: AutoEncoderConfig) -> Float[Tensor, "batch pos n_neurons"]:
    ...
    if cfg.act_name == "mlp.hook_post":
        W_out_U_token = einops.einsum(model.W_out[cfg.layer], W_U_token, "d_mlp d_res, d_res batch pos -> d_mlp batch pos")
...
def get_trigram_token_dla(model: HookedTransformer, encoder: AutoEncoder, encoder_neuron: int, trigram: str, cfg: AutoEncoderConfig):
...
def get_directions_from_dla(dla: Float[Tensor, "n_neurons"], cutoff_dla=0.2, max_directions=3):
    ...
    logging.info(f"Top {max_directions} directions with DLA > {cutoff_dla}: {directions}")
    return directions

def get_trigram_dataset_examples(model: HookedTransformer, trigram: str, german_data: list[str], prompt_length=50, max_prompts=100):
    ...
    logging.info(f"Found {token_prompts.shape[0]} prompts with token '{middle_trigram_token}'")
    logging.info(Counter(occurrences))
    return token_prompts

def get_zero_ablate_encoder_direction_hook(encoder: AutoEncoder, encoder_neuron, cfg: AutoEncoderConfig):
    def zero_feature_hook(value, hook):
        _, x_reconstruct, _, _, _ = custom_forward(encoder, value[:, -1], encoder_neuron, 0)
        value[:, -1] = x_reconstruct
        return value
    return [(cfg.encoder_hook_point, zero_feature_hook)]

def get_encode_activations_hook(encoder: AutoEncoder, encoder_neuron, cfg: AutoEncoderConfig):
...
def get_direction_logit_and_logprob_boost(trigram_tokens: Int[Tensor, "batch pos"], encoder: AutoEncoder, encoder_neuron, model: HookedTransformer, trigram:str, common_tokens: Int[Tensor, "tokens"], all_ignore: Int[Tensor, "tokens"], cfg: AutoEncoderConfig):
    last_trigram_token = model.to_str_tokens(model.to_tokens(trigram))[-1]

    zero_direction_hook = get_zero_ablate_encoder_direction_hook(encoder, encoder_neuron, cfg)
    ...
    last_token_logit_encoded = logits_active[:, last_trigram_token_tokenized].mean(0).item()
    last_token_logit_zeroed = logits_inactive[:, last_trigram_token_tokenized].mean(0).item()
    logging.info(f"Logit '{last_trigram_token}' when reconstructing activations with encoder: {last_token_logit_encoded:.2f}")
    logging.info(f"Logit '{last_trigram_token}' when reconstructing activations with encoder zeroing out the direction: {last_token_logit_zeroed:.2f}")

    logprobs_active = logits_active.log_softmax(dim=-1)
    logprobs_inactive = logits_inactive.log_softmax(dim=-1)
    last_token_logprob_encoded = logprobs_active[:, last_trigram_token_tokenized].mean(0).item()
    last_token_logprob_zeroed = logprobs_inactive[:, last_trigram_token_tokenized].mean(0).item()
    logging.info(f"Logprob '{last_trigram_token}' when reconstructing activations with encoder: {last_token_logprob_encoded:.2f}")
    logging.info(f"Logprob '{last_trigram_token}' when reconstructing activations with encoder zeroing out the direction: {last_token_logprob_zeroed:.2f}")
    ...
    boosted_tokens = model.to_str_tokens(top_tokens[non_zero_boosts]), top_boosts[non_zero_boosts].tolist()
    deboosted_tokens = model.to_str_tokens(top_deboosted_tokens[non_zero_deboosts]), top_deboosts[non_zero_deboosts].tolist()
    logging.info(f"Top boosted: {boosted_tokens}")
    logging.info(f"Top deboosted: {deboosted_tokens}")

    return last_token_logit_encoded, last_token_logit_zeroed, last_token_logprob_encoded, last_token_logprob_zeroed, boosted_tokens, deboosted_tokens

def print_direction_activations(data: list[str], model: HookedTransformer, encoder: AutoEncoder, encoder_neuron: int, cfg: AutoEncoderConfig, mean_feature_activation: float = 5, threshold=0):
    for prompt in data:
        _, cache = model.run_with_cache(
            prompt, names_filter=cfg.encoder_hook_point
            )
...
def get_context_effect_on_feature_activations(model: HookedTransformer, tokens: Int[Tensor, "batch pos"], encoder: AutoEncoder, encoder_neuron: int, deactivate_context_hook, cfg: AutoEncoderConfig):
    context_active_loss = model(tokens, return_type="loss", loss_per_token=True)[:, -1].mean().item()
    _, cache = model.run_with_cache(
        tokens, names_filter=cfg.encoder_hook_point
        )
    ...
    with model.hooks(deactivate_context_hook):
        context_ablated_loss = model(tokens, return_type="loss", loss_per_token=True)[:, -1].mean().item()
    ...
    logging.info(f"Mean feature activation when context neuron active: {feature_activation_context_active:.2f}")
    logging.info(f"Mean feature activation with context neuron inactive: {feature_activation_context_inactive:.2f}")
    return context_active_loss, context_ablated_loss, feature_activation_context_active, feature_activation_context_inactive

def get_encoder_token_reconstruction_losses(tokens: Int[Tensor, "batch pos"], model: HookedTransformer, encoder: AutoEncoder, deactivate_context_hook, cfg: AutoEncoderConfig):
    ...
    with model.hooks(activate_hooks):
        encoder_context_active_loss = model(tokens, return_type="loss", loss_per_token=True)[:, -1].mean()
        logging.info(f"Model loss when patching through encoder with context neuron active: {encoder_context_active_loss.item():.2f}")

    with model.hooks(deactivate_hooks):
        encoder_context_inactive_loss = model(tokens, return_type="loss", loss_per_token=True)[:, -1].mean()
        logging.info(f"Model loss when patching through encoder with context neuron inactive: {encoder_context_inactive_loss.item():.2f}")

    return encoder_context_active_loss, encoder_context_inactive_loss

def get_encoder_feature_reconstruction_losses(tokens: Int[Tensor, "batch pos"], encoder: AutoEncoder, model: HookedTransformer, encoder_neuron: int, feature_activation_context_active: float, feature_activation_context_inactive: float, cfg: AutoEncoderConfig):
    # Set single feature to context active or inactive
    def activate_feature_hook(value, hook):
        _, x_reconstruct, _, _, _ = custom_forward(encoder, value[:, -2], encoder_neuron, feature_activation_context_active)
        value[:, -2] = x_reconstruct
        return value

    def deactivate_feature_hook(value, hook):
        _, x_reconstruct, _, _, _ = custom_forward(encoder, value[:, -2], encoder_neuron, feature_activation_context_inactive)
        value[:, -2] = x_reconstruct
        return value

    def zero_feature_hook(value, hook):
        _, x_reconstruct, _, _, _ = custom_forward(encoder, value[:, -2], encoder_neuron, 0)
        value[:, -2] = x_reconstruct
        return value
    ...
    with model.hooks(activate_hooks):
        loss_encoder_direction_active = model(tokens, return_type="loss", loss_per_token=True)[:, -1].mean()
        logging.info(f"Mean loss when patching second trigram token through encoder: {loss_encoder_direction_active.item():.2f}")

    with model.hooks(deactivate_hooks):
        loss_encoder_direction_inactive = model(tokens, return_type="loss", loss_per_token=True)[:, -1].mean()
        logging.info(f"Mean loss when patching second trigram token through encoder and setting N{encoder_neuron} to activation with context neuron inactive: {loss_encoder_direction_inactive.item():.2f}")

    with model.hooks(zero_ablate_hooks):
        loss_encoder_direction_zeroed = model(tokens, return_type="loss", loss_per_token=True)[:, -1].mean()
        logging.info(f"Mean loss when patching second trigram token through encoder and setting N{encoder_neuron} to zero: {loss_encoder_direction_zeroed.item():.2f}")
    
    return loss_encoder_direction_active, loss_encoder_direction_inactive, loss_encoder_direction_zeroed