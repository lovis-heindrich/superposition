...
def get_unseen_data(seen_filenames: list[str] = []) -> tuple[list[torch.Tensor], int]:
    ...
    def get_pt_files(dirs: list[str]) -> list[str]:
        result = []
...
def main(model_name: str, layer: int, act_name: str, cfg: dict):
    # start_background_data_download() # or start a copy/rsync locally
    ...
    while len(all_data_filenames) < cfg['n_data_files']:
        ...
        num_batches = (
            min([len(item) for item in prompt_data])
            // num_samples_per_batch
        )
        with tqdm(total=num_batches, position=0, leave=True) as pbar:
